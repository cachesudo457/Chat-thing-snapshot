...
into the chat. A small gear (⚙) icon in the top bar opens a full **Settings**
screen — Discord-style, with a list of categories you tap into rather than
one long page:

- **My Account** — change your password. Your old one stops working right
  away.
- **Appearance** — pick a color theme (Sage, Ocean, Sunset, Lavender), a
  font (Rounded, Playful, Serif, Mono), and a message density (Cozy or
  Compact). All three are dropdowns and apply instantly.
- **Notifications** — turn the incoming-message bing sound on or off.
- **Accessibility** — turn off message and screen animations if you'd
  rather things not move around.

Every one of these is a per-browser preference saved in that browser's
`localStorage`, not on the account — so it won't follow you to a
different device or browser, and nobody else sees your choices.

- **Account** — a card right on the dashboard showing your name and
  avatar. The default avatar is the `(ー_ー)` kaomoji; upload your own
  ...
  of its original size. Reset back to the default any time. Persists
  across sessions and applies as soon as you log in.
