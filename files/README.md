...
Friends can call each other with real, live voice — a "Call" button sits
next to each name in your Friends list on the dashboard.

- **How it works:** press Call, and if your friend's online they get a
  ringing screen with Accept/Decline. Once accepted, it's a live one-on-one
  voice call — mic on, with a glowing ring around whoever's currently
  talking (yours or theirs), a mute button, a call timer, and a minimize
  button that shrinks the call to a small bar at the top of the screen so
  you can keep using the rest of the app while still on the call.
- **Who you can call:** only people already on your friends list, and only
  while they're online. You can't have two calls going at once — calling
  someone (or being called) while you're already on a call gets a "busy"
  response.
- **Under the hood:** this is a direct peer-to-peer WebRTC audio connection
  between the two browsers — the server only relays the initial handshake
  (who's calling whom, and the connection details), not the audio itself.
  That keeps it fast and doesn't touch your server's bandwidth, but it also
  means very restrictive networks (some corporate or campus Wi-Fi, certain
  mobile carriers) can occasionally block the direct connection outright,
  since there's no relay/TURN server as a fallback — something to be aware
  of, not something this setup can fully guarantee against.
