...
into the chat. The dashboard has two tabs: **Home** (chat, rooms, friends,
suggestions) and **Settings** (profile, account, look & feel).
...
- **Profile** — the default avatar is the `(ー_ー)` kaomoji. Upload your own
  ...
  log in.
- **Customisation** — also under Settings: a handful of color themes
  (Sage, Ocean, Sunset, Lavender) and fonts (Rounded, Classic, Serif,
  Mono) to pick from. This is a per-browser preference saved in that
  browser's `localStorage`, not on the account — so it won't follow you
  to a different device or browser, and nobody else sees your choice.
