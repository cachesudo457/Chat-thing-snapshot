...
const activeCalls = new Map(); // name -> other party's name, while ringing or connected
...
io.on('connection', socket => {
  ...
  // ---- Voice calls (1:1, WebRTC — this just relays the signaling) ----
  function endActiveCall(myName) {
    const peerName = activeCalls.get(myName);
    activeCalls.delete(myName);
    if (peerName) {
      activeCalls.delete(peerName);
      notify(peerName, 'call-ended', {});
    }
  }

  socket.on('call-user', ({ to: toName }) => {
    if (!authedName) return;
    const me = getAccount(authedName);
    const target = getAccount(toName);
    if (!target) { socket.emit('call-error', { message: 'No account with that name.' }); return; }
    if (target.name.toLowerCase() === me.name.toLowerCase()) return;
    if (!me.friends.some(n => n.toLowerCase() === target.name.toLowerCase())) {
      socket.emit('call-error', { message: 'You can only call friends.' });
      return;
    }
    if (!activeSockets.has(target.name)) {
      socket.emit('call-error', { message: `${target.name} isn't online right now.` });
      return;
    }
    if (activeCalls.has(me.name) || activeCalls.has(target.name)) {
      socket.emit('call-error', { message: `${target.name} is busy right now.` });
      return;
    }
    activeCalls.set(me.name, target.name);
    activeCalls.set(target.name, me.name);
    notify(target.name, 'incoming-call', { from: me.name, avatar: me.avatar });
  });

  socket.on('call-response', ({ accept }) => {
    if (!authedName) return;
    const me = getAccount(authedName);
    const callerName = activeCalls.get(authedName);
    if (!callerName) return;
    if (accept) {
      notify(callerName, 'call-accepted', { by: me.name, avatar: me.avatar });
    } else {
      activeCalls.delete(authedName);
      activeCalls.delete(callerName);
      notify(callerName, 'call-declined', {});
    }
  });

  socket.on('call-signal', ({ data }) => {
    if (!authedName) return;
    const peerName = activeCalls.get(authedName);
    if (!peerName) return;
    notify(peerName, 'call-signal', { from: authedName, data });
  });

  socket.on('call-end', () => {
    if (!authedName) return;
    endActiveCall(authedName);
  });
