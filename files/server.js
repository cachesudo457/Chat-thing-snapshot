...
io.on('connection', socket => {
  ...
  socket.on('set-avatar', ({ type, dataUrl, mime }) => {
    ...
    const myCallId = activeCallOf.get(account.name);
    if (myCallId) {
      const call = calls.get(myCallId);
      if (call) {
        call.members.forEach((m, otherName) => {
          if (otherName === account.name) return;
          m.avatar = account.avatar;
          notify(otherName, 'call-avatar-updated', { callId: myCallId, name: account.name, avatar: account.avatar });
        });
      }
    }
  });
